...
$nisn = only_digits(clean_input($_GET['nisn'] ?? ''));
if ($nisn === '' || strlen($nisn) < 6 || strlen($nisn) > 20) {
    http_response_code(400);
    exit('<div style="padding:20px;text-align:center"><h2>NISN tidak valid</h2><p><a href="index.php">Kembali</a></p></div>');
}
...
try {
    ...
    if (!$siswa) {
        ...
        exit('<div style="padding:20px;text-align:center"><h2>Data siswa tidak ditemukan</h2><p><a href="index.php">Kembali</a></p></div>');
    }
    ...
    $scoreStmt = $pdo->prepare(
        'SELECT mata_pelajaran, nilai
         ...
         ORDER BY urutan ASC, mata_pelajaran ASC'
    );
    $scoreStmt->execute(['siswa_id' => $siswa['id']]);
    $scores = $scoreStmt->fetchAll();
} catch (Exception $e) {
    error_log('Error cetak SKL: ' . $e->getMessage());
    http_response_code(500);
    exit('<div style="padding:20px;text-align:center"><h2>Terjadi kesalahan</h2><p><a href="index.php">Kembali</a></p></div>');
}

function normalize_skl_subject(string $subject): string
{
    $subject = trim($subject);
    if (strcasecmp($subject, 'Muatan Lokal Bahasa Mongondow') === 0) {
        return 'a. Bahasa Mongondow';
    }
    if (strcasecmp($subject, 'Bahasa Mongondow') === 0) {
        return 'a. Bahasa Mongondow';
    }
...
foreach ($scores as $score) {
    $scoresBySubject[normalize_skl_subject((string) $score['mata_pelajaran'])] = $score['nilai'];
}
...
$average = $numericScores ? round(array_sum($numericScores) / count($numericScores)) : '';
...
$nomorSuratSklSiswa = clean_input($siswa['nomor_surat_skl_siswa'] ?? '');
if ($nomorSuratSklSiswa !== '') {
    $nomorSuratSkl = $nomorSuratSklSiswa;
}
...
$status = (string) $siswa['status_kelulusan'];
...
<head>
    ...
    <title>Surat Keterangan Lulus - <?= e($siswa['nama']); ?></title>
    <style>
        * {
            ...
            <table class="scores">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="average-score"><?= e((string) $average); ?></td>
                    </tr>
                </tbody>
            </table>
        </section>
